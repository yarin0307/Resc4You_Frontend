...
const ChatCarMechanics = () => {
  ...
  function storeMessageInFireBase(message) {
    ...
    const reference = ref(
      db,
      "Messages/carMechanics/" + message.timestamp.getTime()
    );
    set(reference, {
      text: message.text,
  ...
  useEffect(() => {
    const db = getDatabase();
    const messagesRef = ref(db, "Messages/carMechanics");
  ...
  const handleSend = () => {
    if (inputText !== "") {
      ...
      flatListRef.current.scrollToEnd({ animated: true });
      sendPushNotificationForRelevantVolunteers(1, newMessage.text);
    }
  };
    ...
    return (
      <View>
        ...
        >
          ...
          <View style={[messageContainerStyle, { marginTop: 5 }]}>
...
export default ChatCarMechanics;
