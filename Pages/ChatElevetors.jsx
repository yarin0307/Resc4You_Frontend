...
const ChatElevators = () => {
  ...
  function storeMessageInFireBase(message) {
    ...
    const reference = ref(
      db,
      "Messages/Elevators/" + message.timestamp.getTime()
    );
    set(reference, {
      text: message.text,
  ...
  useEffect(() => {
    const db = getDatabase();
    const messagesRef = ref(db, "Messages/Elevators");
  ...
  const handleSend = () => {
    if (inputText !== "") {
      ...
      flatListRef.current.scrollToEnd({ animated: true });
      sendPushNotificationForRelevantVolunteers(3, newMessage.text);
    }
  };
